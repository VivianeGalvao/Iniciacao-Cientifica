...
bool Exploratory_Moves(double delta, Particle *sample, int size, int number_function, double* lb, double* ub){
    bool exit = false;
    int i;
...
//            cout<<fitness_global<<"  ";
            ...
            if(!successful){
                ...
                if( functionEvaluations<maxEval ){
                    if(Exploratory_Moves(delta, best_particle, dimension, NUMFUNC, lb, ub)){ delta = 1.5*delta; test = false;}
                    else{
...
//                progenitor[i]->standard_deviation = exp(Normal_distribution(0, 1/sqrt(2*dimension)))*progenitor[i]->standard_deviation*exp(Normal_distribution(0, 1/sqrt(2*sqrt(dimension))));
...
void Evolutionary_Strategy4(int seed, double expected_mean, int dimension, int number_function, double delta, double *x){
    if(delta > 0){
        ...
        for(i=0; i<LAMBDA; i++){
            ...
            progeny[i]->sd = new double[dimension];
        }
        bool success = false;
...
void Evolutionary_Strategy2(int seed, double expected_mean, int dimension, int number_function, double delta, double *x){
    ...
    if(delta > 0){
        ...
        individuo->standard_deviation = Normal_distribution(0, 1/(sqrt(2*dimension)));
       ...
       Upper_Bounds(number_function, dimension, ub);
        ...
        while(functionEvaluations < criteria){
            ...
            individuo->standard_deviation = Normal_distribution(0, 1/sqrt(2*dimension))*individuo->standard_deviation*exp(Normal_distribution(0, 1/sqrt(2*sqrt(dimension))));
